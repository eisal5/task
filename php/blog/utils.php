...
function create_post ($title, $contents, $image) {
    ...
    $db = get_db(); //トランザクションを一連の流れでできる
    $st = $db->prepare($sql);//ここで再度get_db()やるとトランザクションがかぶって情報がおかしいことになる
    $success = $st->execute($params);
    $id = $db->lastInsertId();//最後にインサートした分のIDがとれる

    $sql = "update posts set image_name = ?, image_path = ?, file_type = ? where id =?";
